		      ...
		      <tbody>
			      ...
			      <c:forEach items="${carrinhoCompras.itens }" var="item">
				    <tr>
				          ...
				          <td class="item-title">${item.produto.titulo}
				          <td class="numeric-cell">${item.preco }</td>
				          <td class="quantity-input-cell">
				          		<input type="number" min="0" 
				          		readonly="readonly" id="quantidade"
				          		 name="quantidade" value="${carrinhoCompras.getQuantidade(item) }"/>
				          	</td>
			      ...
			      <tfoot>
			        <tr>
			          <td colspan="3">
			          <!-- {s:mvcUrl('PC#finalizar').build() }  PC significa as iniciais de PagamentoController -->
			          	<form action="${s:mvcUrl('PC#finalizar').build() }" method="post">
			          		<input type="submit" class="checkout" name="checkout" id="checkout" value="Finalizar compra" />
			          	</form>
			          	</td>
